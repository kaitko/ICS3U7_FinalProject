...
public class MappingPane extends JPanel implements ActionListener{
	...
	// configurations
	private ImageIcon[] icon = {new ImageIcon("player1.jpg"), new ImageIcon("player2.jpg")};
	...
	private void processClick(int i, int j){
		...
		if (!hasWinner()) { 						//check for winner, if none found
			...
			btnGrids[i][j].setIcon(icon[player-1]);	//set current icon

			if (player == 1) {						//bot turn
				try {TimeUnit.MILLISECONDS.sleep(1000);} catch (InterruptedException ex) {} //pause bot
				moveByBot();
			}  
		}
	}
	...
	private void moveByBot() {
		...
		if (row == rows - 1) {	//if current player is at most bottom row
			...
			//calculate whether to win or not based on chance related level selection (winning objective)
			int toWin = (int) (Math.random()*100) < cbxLevel.getSelectedIndex() * 25 + 25 ? 1: 0;

			//for each possible step, find the first move that matches winning objective
			for (int i = 0; i<3 ; i++) { 	
				x = nextMove[i][0]; 		//set horizontal coordinates
				y = nextMove[i][1];			//set vertical coordinates
				
				//if rows-x and cols-y are both odd numbers, the (x, y) spot would be a winning position
				if (((rows - x) * (cols - y) % 2) == toWin) //if move matches winning objective 
					break;
			}
		}
		
		btnGrids[row][col].setIcon(null); 		//remove previous move icon
		btnGrids[x][y].setIcon(icon[player-1]); //set new move icon
	...
	private boolean hasWinner(){
		if (row == rows - 1 && col == cols - 1) { //found a winner
			showMessageDialog(this, (player ==1? "Too bad, Botty": "Congratulations! You") + " won!");//winner dialogue
	...
	public void actionPerformed(ActionEvent e){
		...
		if (source == btnGO) {
			...
			// read config
			rows = gameSize[cbxSize.getSelectedIndex()][0]; 
			...
			btnGrids[row][col].setIcon(icon[1]);
